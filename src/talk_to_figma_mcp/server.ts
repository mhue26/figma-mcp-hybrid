...
function restError(error: unknown) {
  return {
    isError: true,
    content: [
      {
  ...
  "post_figma_comment",
  {
    ...
    inputSchema: {
      ...
      nodeId: z
        .string()
        .optional()
        .describe("Optional node ID to anchor the comment to"),
    },
...
// Start the server
async function main() {
  // REST read tools need a token; the plugin write tools do not. Warn but
  // don't exit, so the WebSocket bridge still works without a token.
  if (!hasFigmaToken()) {
    logger.warn(
      "FIGMA_TOKEN is not set. REST read tools (get_figma_file, etc.) will return an error until it is configured. Plugin/WebSocket tools are unaffected."
    );
  }
